...
CREATE TABLE `t_robotstatusrecord` (
  ...
  `location` varchar(255) DEFAULT NULL COMMENT '机器人位置 JSON 文本，例如 {"x":12.3,"y":5.6}',
...
CREATE TABLE `robot_current_state` (
  ...
  `current_site_id` varchar(64) DEFAULT NULL COMMENT '当前匹配到的地图节点编码，对应 t_mapnode.node_code；机器人不在节点上时允许为空',
  `current_x` decimal(12,4) DEFAULT NULL COMMENT '机器人当前地图 X 坐标，用于匹配最近可接入地图节点',
  `current_y` decimal(12,4) DEFAULT NULL COMMENT '机器人当前地图 Y 坐标，用于匹配最近可接入地图节点',
